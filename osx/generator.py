...
input_image_path = "images/dress_image.jpeg"  # Your reference dress image
...
mac_steps = 25  # Reduced steps for better Mac performance
mac_batch_size = 1  # Fixed batch size for Mac
...
try:
    ...
    if width > 1024 or height > 1024:
        ...
        input_img.resize((new_width, new_height), Image.LANCZOS).save(resized_path, quality=95)
...
# Generate variations with Mac optimization
for i in range(num_variations):
    ...
    # Make API request with Mac optimization
    try:
        print(f"  Prompt: {prompt}")
        print(f"  Strength: {strength}")
        print(f"  Steps: {mac_steps} (optimized for Mac)")
        ...
        with open(input_image_path, 'rb') as img_file:
            ...
            data = {
                ...
                'guidance_scale': '7.0',
                'steps': str(mac_steps),  # Mac-optimized step count
                'batch_size': str(mac_batch_size)  # Always 1 for Mac
            }
            
            # Add timeout to prevent hanging indefinitely
            response = requests.post(api_url, files=files, data=data, timeout=300)  # Longer timeout for Mac
        ...
        print("  Error: Request timed out. The server may be overloaded.")
    ...
    # Longer delay for Mac to allow cooling and memory cleanup
